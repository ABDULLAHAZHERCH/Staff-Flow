  ...
  useEffect(() => {
    const fetchReportData = async () => {
      try {
        ...
        const response = await fetch(`/api/reports?type=hours&startDate=${startDate}&endDate=${endDate}`)
        ...
        const chartData = Object.entries(reportData.report.departmentHours).map(([name, hours]) => ({
          name,
          hours: Number(hours),
        }))
    ...
    <ChartContainer>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data}>
          ...
          />
          <Legend />
          <Bar dataKey="hours" name="Hours" fill="#03306b" radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </ChartContainer>
  )
