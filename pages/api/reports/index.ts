...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (decoded.role !== "manager" && req.query.type !== "employee") {
    return res.status(403).json({ message: "Forbidden: Only managers can access reports" })
  }
    ...
    case "GET":
      try {
        ...
        const filteredShifts = department
          ? shifts.filter((shift: any) => shift.employee.department === department)
          : shifts
        ...
        if (type === "hours") {
          ...
          const employeeHours: { [key: string]: { name: string; hours: number; department: string } } = {}
          ...
          filteredShifts.forEach((shift: any) => {
            ...
            if (!employeeHours[empId]) {
              employeeHours[empId] = { name: empName, hours: 0, department: dept }
            }
          ...
          report = {
            ...
            totalHours: filteredShifts.reduce((sum: number, shift: any) => sum + (shift.totalHours || 0), 0),
          }
        } else if (type === "payroll") {
          // Payroll report - Group by employee or department
          const departmentPayroll: { [key: string]: number } = {}
          const employeePayroll: { [key: string]: { name: string; pay: number; department: string } } = {}
          ...
          filteredShifts.forEach((shift: any) => {
            ...
            if (!employeePayroll[empId]) {
              employeePayroll[empId] = { name: empName, pay: 0, department: dept }
            }
          ...
          report = {
            ...
            totalPayroll: filteredShifts.reduce((sum: number, shift: any) => sum + (shift.totalPay || 0), 0),
          }
        } else if (type === "employee") {
          // Employee report - Only for the current employee
          if (decoded.role !== "employee" && !employee) {
            return res.status(400).json({ message: "Employee ID is required for employee reports" })
          }

          const employeeId = employee || decoded.id
          const employeeShifts = filteredShifts.filter((shift: any) => shift.employee._id.toString() === employeeId)

          const totalHours = employeeShifts.reduce((sum: number, shift: any) => sum + (shift.totalHours || 0), 0)
          const totalPay = employeeShifts.reduce((sum: number, shift: any) => sum + (shift.totalPay || 0), 0)

          report = {
            shifts: employeeShifts,
            totalHours,
            totalPay,
            averageHoursPerShift: employeeShifts.length ? totalHours / employeeShifts.length : 0,
          }
        } else {
